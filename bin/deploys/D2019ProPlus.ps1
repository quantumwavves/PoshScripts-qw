function 2019Deploy {
    ...
    (New-Object System.Net.WebClient).DownloadFile($xml, "$env:temp\2019PP.xml")
    ...
    $currentStep++
     ...
     if (Test-Path -Path "$env:temp\2019PP.xml"){
    ...
    cmd.exe /c "$env:temp\deploy\setup.exe /configure $env:temp\2019PP.xml"
    $currentStep++
    #Cleaning temp files
    ...
    Remove-Item "$env:temp\deploy" -Recurse -Force
    Remove-Item "$env:temp\officeDeploy.exe" -Force
    Remove-Item "$env:temp\2019PP.xml" -Force
    #Finished deploy
